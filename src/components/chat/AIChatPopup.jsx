        ...
        <>
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        {/* Header */}
                        <div className="bg-teal-600 dark:bg-teal-700 px-4 py-3 flex items-center justify-between text-white">
                            <div className="flex items-center gap-3">
                                <div className="relative">
                                    ...
                                    {/* Online indicator */}
                                    <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-400 rounded-full border-2 border-teal-600 dark:border-teal-700" />
                                </div>
                                <div>
                                    <h3 className="font-bold text-base leading-tight tracking-wide">Voyage AI</h3>
                        ...
                        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-3 bg-slate-50 dark:bg-slate-950" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                            {messages.map((msg, index) => (
                                ...
                                >
                                    {/* Bot avatar */}
                                    {msg.role === "assistant" && (
                                        <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0 ring-1 ring-teal-200 dark:ring-teal-800 shadow-sm mb-1">
                                    ...
                                    <div className="max-w-[80%] flex flex-col">
                                        {/* Quoted reply block */}
                                        {msg.replyTo && (
                                            <div className={`mb-1 px-3 py-1.5 rounded-lg border-l-2 border-teal-500 text-[11px] leading-snug ${msg.role === "user"
                                        ...
                                        <div
                                            className={`px-3.5 py-2.5 text-[13px] leading-relaxed ${msg.role === "user"
                                                ? "bg-teal-600 dark:bg-teal-700 text-white rounded-2xl rounded-br-md shadow-sm prose prose-sm prose-invert max-w-none"
                            ...
                            {replyingTo && (
                                ...
                                >
                                    <div className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex items-center gap-2">
                                        <div className="flex-1 min-w-0 border-l-2 border-teal-500 pl-2.5">
                        ...
                        <div className="px-3 py-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
                            <form onSubmit={handleSendMessage} className="relative flex items-center">
                                <input
                                    ...
                                    className="w-full pl-4 pr-12 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/40 focus:border-teal-400 dark:focus:ring-teal-400/30 dark:focus:border-teal-500 text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 text-sm transition-all"
                                />
                                <button
                                    type="submit"
                                    disabled={!inputText.trim() || isLoading}
                                    className="absolute right-1.5 w-9 h-9 bg-teal-600 dark:bg-teal-700 text-white rounded-lg flex items-center justify-center hover:bg-teal-700 dark:hover:bg-teal-600 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-105 active:scale-95 transition-all shadow-sm"
                            ...
                            <p className="text-center mt-2 text-[10px] text-slate-400 dark:text-slate-400">
                                Powered by Voyage AI · May produce inaccurate results
                            </p>
                        </div>
                    </motion.div>
                )}
            ...
            {(!isOpen && isVisible) && (
                <motion.button
                    ...
                    onClick={toggleChat}
                    className="fixed bottom-8 right-8 w-16 h-16 rounded-full shadow-2xl shadow-teal-900/25 dark:shadow-black/40 z-50 hover:shadow-teal-900/35 transition-shadow group overflow-hidden ring-2 ring-white/80 dark:ring-slate-700"
