...
====================================================== */
const upload = createUpload("voyage/packages", "image");
...
router.post("/", adminAuth, upload, async (req, res) => {
  try {
    ...
    const imagePath = req.file.path; // ☁️ Cloudinary URL
...
/* ======================================================
🔧 FIX IMAGE PATHS (Admin Migration - One Time)
POST /api/packages/fix-image-paths
====================================================== */
const fs = require("fs");

router.post("/fix-image-paths", adminAuth, async (req, res) => {
  try {
    ...
    for (const pkg of packages) {
      ...
      // Remove extra spaces from path
      const cleanPath = pkg.image.replace(/\s+/g, "").replace(/\//g, "/");
      // Normalize to proper format: "uploads/package-TIMESTAMP.ext"
      const normalizedPath = cleanPath
        .replace("uploads/", "uploads/")
        .replace(/^uploads/, "uploads");

      if (normalizedPath !== pkg.image) {
        // Try to find and rename the actual file on disk
        const oldFilename = pkg.image.replace(/.*\//, "").trim();
        const newFilename = normalizedPath.replace(/.*\//, "").trim();
        const uploadsDir = path.join(__dirname, "..", "uploads");

        // Check if old file exists (with spaces)
        ...
        if (fs.existsSync(oldFullPath) && oldFullPath !== newFullPath) {
          try {
            ...
            console.warn(`⚠️ Could not rename file: ${renameErr.message}`);
          }
        }

        pkg.image = normalizedPath;
        await pkg.save();
        fixedCount++;
        console.log(`✅ Fixed: ${pkg.name} → ${normalizedPath}`);
      }
    }

    res.json({
      success: true,
      message: `Fixed ${fixedCount} package image paths out of ${packages.length} total.`,
    });
  } catch (err) {
    console.error("❌ Fix Image Paths Error:", err.message);
